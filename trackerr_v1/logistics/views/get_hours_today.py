...
class GetHoursToday(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            # set cache
            cache.set(f'hours_today:{rider.id}', get_time, timeout=10)
            return Response({'msg': f'{get_time}'}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'msg': 'an internal server error occurred!'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
