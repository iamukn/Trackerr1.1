...
#        self.wallet = Wallet.objects.create(owner=self.user, balance=150.00, currency='NGN')
    ...
    def test_retrieve_all(self, mock_track_gen, mock_verify, mock_deduct):
        ...
        mock_deduct.return_value = 150.0
    ...
    @patch('tracking_information.views.generate_tracking_view.Track_gen')
    @patch('tracking_information.views.generate_tracking_view.verify_address')
    def test_retrieve_one(self, mock_verify, mock_track_gen, mock_wallet):
        mock_track_gen_instance = MagicMock(return_value=self.track_num)
        #mock_apply_async_instance = MagicMock(return_value=self.return_value)
        mock_track_gen.return_value.generate_tracking = mock_track_gen_instance
        mock_verify.return_value = self.return_value
    ...
    @patch('tracking_information.views.generate_tracking_view.verify_address')
    @patch('tracking_information.views.generate_tracking_view.Track_gen')
    def test_retrieve_all_for_a_customer(self, mock_track_gen, mock_verify, mock_wallet):
        mock_track_gen_instance = MagicMock(return_value=self.track_num)
        #mock_apply_async_instance = MagicMock(return_value=self.return_value)
        mock_track_gen.return_value.generate_tracking = mock_track_gen_instance
        mock_verify.return_value = self.return_value
